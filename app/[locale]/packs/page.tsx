...
import { packs, getPackName, getPackDesc } from '@/lib/packs';
...
function PackCard({ pack, onSelect }: { pack: (typeof packs)[0]; onSelect: (pack: typeof packs[0]) => void }) {
...
function BoxBuilder({ pack, onClose }: { pack: typeof packs[0]; onClose: () => void }) {
  const locale = useLocale();
...
function CustomBoxBuilder() {
  ...
  const pricePerCookie = size === 6 ? 3 : size === 12 ? 2.83 : 2.67;
  const boxPrice = size === 6 ? 18 : size === 12 ? 34 : 48;
      ...
      <div className="flex gap-3 mb-8">
        {([6, 12, 18] as const).map(s => (
          <button key={s} onClick={() => handleSizeChange(s)}
            ...
            {s} cookies<br />
            <span className="font-bold text-base">€{s === 6 ? 18 : s === 12 ? 34 : 48}</span>
          </button>
        ))}
      </div>
...
export default function PacksPage() {
  ...
  return (
    <div className="min-h-screen bg-cream pb-20">
      {selectedPack && <BoxBuilder pack={selectedPack} onClose={() => setSelectedPack(null)} />}
